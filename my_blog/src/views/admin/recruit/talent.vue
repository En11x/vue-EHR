	...
	<div class="talent">
		<p>
			<el-breadcrumb separator="/">
				...
				<el-breadcrumb-item>人才储备管理</el-breadcrumb-item>
			</el-breadcrumb>
		</p>
		<div class="search">
			<div>
		...
		<div class="btns">
			...
			<el-button type="warning" size="mini" icon="el-icon-arrow-right">面试邀请</el-button>
		</div>
		<div class="article-table-wrap">
			<div class="table" v-loading='loading'>
				<el-table :data="tableData" border stripe style="width: 100%" size="mini" @selection-change="handleSelectionChange">
					...
					<el-table-column prop="specialty" label="技术特长" min-width="120" align="center"></el-table-column>
					<el-table-column prop="comments" label="圈内评价" show-overflow-tooltip align="center" min-width='100'></el-table-column>
					<el-table-column prop="asTheCompany" label="先前所在公司" show-overflow-tooltip align="center" min-width="80"></el-table-column>
		...
		<el-dialog :title="editForm.id?'修改':'添加'" :visible.sync="dialogFormVisible">
			<el-form size="mini" :model="editForm">
				...
				<el-form-item label="技术特长" label-width="120px">
					<el-input v-model="editForm.specialty" :placeholder="editForm.specialty" autocomplete="off"></el-input>
				</el-form-item>
				<el-form-item label="圈内评价" label-width="120px">
					<el-input v-model="editForm.comments" :placeholder="editForm.comments" autocomplete="off"></el-input>
				</el-form-item>
                <el-form-item label="先前所在公司" label-width="120px">
					<el-input v-model="editForm.asTheCompany" :placeholder="editForm.asTheCompany" autocomplete="off"></el-input>
				</el-form-item>
...
export default {
	...
	methods: {
		getAllTalent() {
			this.$axios.get('/talent/queryTalent').then(res => {
		...
		handleDelete(index, row, datas) {
			...
			this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
				...
				.then(() => {
					this.$axios
						.post('/talent/deleteTalent', { ids: ids })
						.then(res => {
							if (res.data.success) {
								// 删除成功
		...
		submit() {
			...
			this.updateDatas.TalentTime = this.$moment(
				this.updateDatas.TalentTime
			).format('YYYY-MM-DD')
			...
			if (this.updateDatas.id) {
				...
				this.$axios
					.post('/talent/updateTalent', this.updateDatas)
					.then(res => {
						if (res.data.success) {
							// 编辑成功
				...
				if (
					this.allDatas.find(item => item.name == this.editForm.name)
				) {
				...
				//增加数据
				this.$axios
					.post('/talent/addTalent', this.updateDatas)
					.then(res => {
						if (res.data.success) {
							// 添加成功
		...
		search() {
			...
			let sex = this.searchSex.trim() == '男' ? 0 : 1
